...
# --- KEY CHANGE: Theme-Aware CSS for Highlighting ---
st.markdown("""
<style>
    /* Define highlight color for Light Theme */
    [data-theme="light"] {
        --highlight-background-color: #d4edda; /* Soft Green */
    }
    /* Define highlight color for Dark Theme */
    [data-theme="dark"] {
        --highlight-background-color: #334155; /* Muted Blue-Grey */
    }
</style>
...
def manual_pairing(matched, unmatched_left, unmatched_right, left_files, right_files):
    ...
    initial_left_names = [f.name for f in left_files]
    initial_right_names = [f.name for f in right_files]
...
def compare_sheets_fast(df1, df2, key_columns):
    ...
    df1_norm = normalize_df_fast(df1.copy()).set_index(key_columns)
    df2_norm = normalize_df_fast(df2.copy()).set_index(key_columns)
    ...
    added_rows = df2.set_index(key_columns).loc[added_indices].reset_index() if not added_indices.empty else pd.DataFrame(columns=df2.columns)
    removed_rows = df1.set_index(key_columns).loc[removed_indices].reset_index() if not removed_indices.empty else pd.DataFrame(columns=df1.columns)
...
def run_comparison_computation():
    ...
    try:
        ...
        st.session_state.report_buffer = generate_excel_report(all_results, key_columns)
...
def generate_excel_report(all_results, key_columns):
        ...
        for result in all_results:
            ...
            for res in result["sheets"]:
                ...
                if not res['changed_new'].empty: 
                    # For excel, we still want a light color for highlighting
                    highlight_fill = PatternFill(start_color="C6EFCE", end_color="C6EFCE", fill_type="solid") # Light Green
                    sheet_name = f"P{i}_{res['name']}_Changed"[:31]
                    new_df = res['changed_new']
                    old_df = res['changed_old']
                    new_df.to_excel(writer, sheet_name=sheet_name, index=False)
                    ws = writer.sheets[sheet_name]

                    # Add highlights to excel
                    old_df_indexed = old_df.set_index(key_columns)
                    new_df_indexed = new_df.set_index(key_columns)
                    for r_idx, (idx, row) in enumerate(new_df_indexed.iterrows(), 2):
                        old_row = old_df_indexed.loc[idx]
                        for c_idx, col_name in enumerate(new_df_indexed.columns, 1):
                            if str(row[col_name]) != str(old_row[col_name]):
                                ws.cell(row=r_idx, column=c_idx + len(key_columns)).fill = highlight_fill
