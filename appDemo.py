    ...
    try:
        ...
        for i, (lf, rf) in enumerate(st.session_state.pairs, 1):
            ...
            try:
                ...
                if key_columns:
                    cols1, cols2 = set(df1.columns), set(df2.columns)
                    if set(key_columns).issubset(cols1) and set(key_columns).issubset(cols2):
                        use_key_based = True
...
# --- UI: PHASE 1 (SETUP) ---
    ...
    if left_files and right_files:
        ...
        st.text_input(
            ...
            help="Provide unique ID column(s) to find and highlight changed rows. If blank, only added/removed rows will be shown.",
            key='key_columns_str'
        )
